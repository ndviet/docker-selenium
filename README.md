...
| default (uid != `1200`) | default `/home/seluser/Downloads`    | Yes           |
| default (uid != `1200`) | any `/path/to/downloads`             | No            |
...
**Note:** For this option to take effect with running non-root or arbitrary user ids, you must run the container with `--user root` once before switching to non-root user (same as the section `Mounting volumes to retrieve downloaded files`).
